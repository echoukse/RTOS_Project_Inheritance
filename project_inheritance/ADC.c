...
TODO : include argument for sequencer
*/
// This initialization function sets up the ADC0 according to the
...
void ADC_Init(uint32_t channelNum)
 ...
 switch(channelNum)
	...
	//SYSCTL_RCGC0_R |= 0x00010000;   // 7) activate ADC0 (legacy code)   //TODO check 
...
//SYSCTL_RCGC0_R &= ~0x00000300;  // 9) configure for 125K (legacy code)
  ADC1_PC_R &= ~0xF;              // 9) clear max sample rate field
	ADC1_PC_R = 0x01;               // 9) configure for 125K samples/sec
...
uint32_t ADC_In(void){  uint32_t result;
  ...
  result = ADC1_SSFIFO3_R&0xFFF;   // 3) read result only onve since sequence 3
...
void ADC_Collect(uint8_t channelNum, int freq, void(*task)(unsigned long)){
  ...
  TIMER1_CTL_R |= 0x00000001;   // enable timer1A 16-b, periodic, no interrupts
...
void ADC0Seq3_Handler(void){ 
  ...
  ADCval = ADC0_SSFIFO3_R;  // 12-bit result
	//ADCTask(ADCval); //## adding producingas os thread
	OS_AddThread_param(ADCTask,128,0,ADCval);
}
