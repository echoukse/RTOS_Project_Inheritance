...
unsigned long myId = OS_Id(); 
  ...
  ST7735_Message(1,1,"PIDWork     =",PIDWork);
	ST7735_Message(1,2,"Jitter 0.1us=",MaxJitterper[0]);
	ST7735_Message(1,3,"Jitter 0.1us=",MaxJitterper[1]);
...
unsigned long data,voltage;
  ...
  ST7735_Message(0,1,"Run length = ",(RUNLENGTH)/FS);   //##uncomment top half used for Display
  while(NumSamples < RUNLENGTH) { 
		data = OS_MailBox_Recv();
		voltage = 3000*data/4095;               // calibrate your device so voltage is in mV
    PE3 = 0x08;
		//ST7735_Message(0,2,"NumSamples =",NumSamples); //##uncomment  
    //ST7735_Message(0,2,"v(mV) =",voltage); //##uncomment  
    PE3 = 0x00;
  } 
    ...
    Command=UART_InUDec();
		...
		switch(Command){
			case 1: 
			  ...
			  UART_OutUDec(NumCreated);OutCRLF();
				UART_OutString("PID Work");
			  UART_OutUDec(PIDWork);OutCRLF();
				UART_OutString("FilterWork");
			  UART_OutUDec(FilterWork);OutCRLF();
				UART_OutString("NumSamples");
			  UART_OutUDec(NumSamples);OutCRLF();
			  break;
			case 2:
				//Output_Init();
				UART_OutString("Debug param X");OutCRLF();
			  for(int i=0;i<64;++i)
			   {
					 UART_OutUDec(x[i]);
					 UART_OutString(" , ");
				...
				OutCRLF();
				UART_OutString("Debug param Y");OutCRLF(); 
				for(int i=0;i<64;++i)
			   {
					 UART_OutUDec(y[i]);
					 UART_OutString(" , ");
			  ...
			  break;
			case 3:
				//Timerval=((Timer2Rollover*TIME_2MS+(TIME_2MS-OS_Time()))/80);
			  Timerval= OS_Time();
			  UART_OutString("Timer reads (in us) :");
			  UART_OutUDec(Timerval);OutCRLF();
        break;			
			case 4:
				for(int i = 0; i<100; i++){
					switch(myevents[i].ID){
						case -1: UART_OutString("Periodic Thread 1 Started :");
						UART_OutUDec(myevents[i].time);OutCRLF();
						break;
						case -2: UART_OutString("Periodic Thread 1 Ended :");
						UART_OutUDec(myevents[i].time);OutCRLF();
						break;
						case -3: UART_OutString("Periodic Thread 2 Started :");
						UART_OutUDec(myevents[i].time);OutCRLF();
						break;
						case -4: UART_OutString("Periodic Thread 2 Ended :");
						UART_OutUDec(myevents[i].time);OutCRLF();
						break;
						default: UART_OutString("Thread " );UART_OutUDec(myevents[i].ID); UART_OutString(" Scheduled :");
						UART_OutUDec(myevents[i].time);OutCRLF();
						break;				
...
void TaskB(void);
 ...
 void thread_p1(void){  // foreground thread - highest priority       
  for(;;){
		OS_Block_bWait(&mutex1); // gets blocked the first time control reaches here 
    OS_Block_bWait(&mutex2); 
		LEDS |= GREEN;
		for(int i=0;i<50;++i){};
		OS_Block_bSignal(&mutex2);
   	OS_Block_bSignal(&mutex1);
		LEDS &= ~GREEN;	
  }
}
...
void thread_p4(void){  // foreground thread        
  for(;;){
		LEDS |= BLUE;
		for(int i=0;i<2;++i){};
		LEDS &=~BLUE;	
...
void thread_p3(void){  // foreground thread        
  for(;;){
		OS_Block_bWait(&mutex2);   // acquire mutex2 
    OS_Block_bSignal(&mutex1);  //signal mutex 1 to wake up thread_p1
		LEDS |= RED;
		NumCreated += OS_AddThread(&thread_p2,128,2); // now control goes to thread_p2 - priority inversion
		NumCreated += OS_AddThread(&thread_p4,128,0); 
...
int main(void){ 
  ...
  MaxJitter = 0;       // in 1us units
	OS_InitSemaphore(&mutex1,0); //mutex is initialized to 0 to create the situation for inversion.
  OS_InitSemaphore(&mutex2,1);
...
//*******attach background tasks***********
  ...
  //NumCreated += OS_AddThread(&Consumer,128,1); 
	...
	//OS_AddPeriodicThread(&DAS,PERIOD,0,1); // 2 kHz real time sampling of PD3
	//OS_AddPeriodicThread(&TaskB,0.75*TIME_1MS,1,2);         // 2 ms, lower priority
...
int main1(void){       // Testmain5 Lab 3
  OS_Init();           // initialize, disable interrupts
	UART_Init();
	PortE_Init();
  PortF_Init();
...
int main3(void){      // Testmain6  Lab 3
  ...
  NumCreated += OS_AddThread(&Wait3,128,2); 	// waiting thread
	...
	NumCreated += OS_AddThread(&Interpreter,128,2); 	// waiting thread
