...
//------------ST7735_Message------------
// String draw function, in split window.  
// Input: device    top or bottom half
//        line      0-3
//        string    pointer to a null terminated string to be printed
//        value     a number to be printed
// Output: number of characters printed
uint32_t ST7735_Message (int device, int line, char *string, unsigned long value)
{ 
	uint32_t count = 0;
	uint16_t x = 0;
	char *valstr;
  ...
  while(*string){
		OS_bWait(&LCD_free);
    ST7735_DrawCharS(6*x, device*8*8 + line*8, *string, ST7735_WHITE, ST7735_BLACK, 1);
    OS_bSignal(&LCD_free);
		string++;
    x = x+1;
    if(x>20) return count;  // number of characters printed
	...
	while(*valstr){
		OS_bWait(&LCD_free);
    ST7735_DrawCharS(6*x, device*8*8 + line*8, *valstr, ST7735_WHITE, ST7735_BLACK, 1);
    OS_bSignal(&LCD_free);
		valstr++;
    x = x+1;
    if(x>20) return count;  // number of characters printed
